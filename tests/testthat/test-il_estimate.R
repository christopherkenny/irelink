...
test_that('il_estimate_prior() with higher recall gives higher prior', {
  ...
  # recall=0.9 should give a higher prior than recall=1.0
  # because it assumes we're missing some matches
  model_r10 <- il_model(df, spec = spec, con = con) |>
    il_estimate_prior(block_on(first_name), recall = 1.0)

  model_r09 <- il_model(df, spec = spec, con = con) |>
