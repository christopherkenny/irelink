...
[![R-CMD-check](https://github.com/christopherkenny/irelink/actions/workflows/R-CMD-check.yaml/badge.svg)](https://github.com/christopherkenny/irelink/actions/workflows/R-CMD-check.yaml)
<!-- badges: end -->
...
match-probability threshold. The returned `match_weight` is the
evidence-only log2 Bayes factor; `total_match_weight` adds the prior
odds used to compute `match_probability`. `il_cluster()` groups the
matched pairs into deduplicated entities.
...
As in the prior example, `il_cleanup()` drops this model’s temporary
tables.
