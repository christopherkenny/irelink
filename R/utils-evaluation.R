...
score_labeled_pairs <- function(model, labels) {
  ...
  found <- label_blocking_flags(model, labels, dialect)
...
#' Evaluate whether labeled pairs satisfy any prediction-time blocking rule
#'
#' @param model A trained `il_model`.
#' @param labels A data frame with `unique_id_l`, `unique_id_r`, `is_match`.
#' @param dialect SQL dialect string.
#' @return Logical vector indicating whether each labeled pair would be
#'   generated by the model's blocking rules.
#' @noRd
label_blocking_flags <- function(model, labels, dialect = detect_dialect(model$con)) {
  blocking_rules <- model$spec$blocking_rules
  if (length(blocking_rules) == 0L) {
    return(rep(TRUE, nrow(labels)))
  }
  ...
  lbl_tbl <- il_table_name(model, 'eval_blocking_labels', il_table_suffix())
  lbl_df <- data.frame(
    pair_idx = seq_len(nrow(labels)),
    uid_l = as.character(labels$unique_id_l),
    uid_r = as.character(labels$unique_id_r),
    stringsAsFactors = FALSE
  )
  DBI::dbWriteTable(con, lbl_tbl, as.data.frame(lbl_df), overwrite = TRUE)
  on.exit(drop_registered(con, lbl_tbl), add = TRUE)
  ...
  sql <- glue::glue(
    'SELECT lbl.pair_idx, {br_sql} AS found_by_blocking ',
    'FROM {sql_quote_identifier(lbl_tbl)} lbl ',
    'JOIN {qtbl_l} l ON CAST(l.unique_id AS VARCHAR) = lbl.uid_l ',
