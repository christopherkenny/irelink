...
il_estimate_em <- function(
  ...
  if (fix_u && fix_m) {
    cli::cli_abort(
      'At least one of {.arg fix_u} and {.arg fix_m} must be {.code FALSE}.'
    )
  }
  ...
  if (length(trained_names) > 0 && length(deact_names) > 0) {
    ...
    cli::cli_inform(
      'EM: all comparisons overlap with the blocking rule; nothing was trained.'
    )
  }
  ...
  for (iter in seq_len(max_iter)) {
    ...
    if (!fix_prior) {
      if (!is.null(prior_info$prevalence)) {
        alpha <- prior_info$prevalence$probability *
          prior_info$prevalence$strength
        beta <- (1 - prior_info$prevalence$probability) *
          prior_info$prevalence$strength
    ...
    if (!fix_m) {
      for (j in seq_len(n_comp)) {
        ...
        for (k in seq(0L, nl - 1L)) {
          ...
          raw[k + 1L] <- (sum(weights[mask] * pattern_n[mask]) +
            0.5 / nl +
            prior_alpha[k + 1L]) /
        ...
        raw <- pmax(raw, 0.001)
        raw <- raw / sum(raw)
        raw <- apply_m_constraint(
          raw,
          prior_info$m_constraints[[comp_names[j]]]
        )
        m_list[[j]] <- raw
      }
    } else {
    ...
    if (!fix_u) {
      for (j in seq_len(n_comp)) {
        ...
        for (k in seq(0L, nl - 1L)) {
          mask <- pattern_mat[, j] == k
          raw[k + 1L] <- (sum((1 - weights[mask]) * pattern_n[mask]) +
            0.5 / nl) /
            (sum_nw + 0.5)
        }
        raw <- pmax(raw, 0.001)
    ...
    history[[iter]] <- tibble::tibble(
      ...
      gamma_level = unlist(lapply(levels_per_comp, function(nl) {
        seq(0L, nl - 1L)
      })),
      value = unlist(m_list)
    )
