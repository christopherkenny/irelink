...
zero_zero_timings <- tibble::tibble(
  ...
  seconds = c(
    ...
    zero_zero_blocking_time + zero_zero_fit_time + zero_zero_accuracy_time
  )
)
...
The same specification is used here, but the default-noise file makes the EM updates materially less clean than in No noise to zero`.
...
This is the hardest one-sided linkage task in the document, so the fitted parameter separation is weaker than in No noise to default`.
...
zero_high_timings <- tibble::tibble(
  ...
  seconds = c(
    ...
    zero_high_blocking_time + zero_high_fit_time + zero_high_accuracy_time
  )
)
