...
`il_suggest_blocking()` enumerates columns and ranks them as blocking
keys.
A good blocking key has high `n_distinct` (narrow blocks, fewer pairs)
and high `coverage` (few missing values):
...
The saved file stores the spec and trained parameters so you can
re-apply the model without retraining:
...
representative sample, save the model, and re-apply as new data
arrives.
